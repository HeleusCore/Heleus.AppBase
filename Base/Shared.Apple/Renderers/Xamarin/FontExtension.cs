...
namespace Heleus.Apps.Shared.Apple.Renderers
{
	public static class FontExtensions
	{
...
#endif
		{
			...
			return ToUIFont(self.FontFamily, size, self.FontAttributes);
		}
...
#endif
		{
            ...
            var size = (float)(double)label.GetValue(Label.FontSizeProperty);
            var attr = (FontAttributes)label.GetValue(Label.FontAttributesProperty);

			return ToUIFont(family, size, attr) ?? UIFont.SystemFontOfSize(UIFont.LabelFontSize);
		}
		...
		{
            var f = FontStyleConverter.GetFontByStyle(family, size);
            if (f != null)
                return f;
		...
		internal static UIFont ToUIFont(string family, float size, FontAttributes attributes)
		{

...
namespace Heleus.Apps.Shared.macOS.Renderers
{
	internal class ListViewDataSource : NSTableViewSource
	{
        ...
        readonly Dictionary<string, WeakReference<Cell>> _nativeCells = new Dictionary<string, WeakReference<Cell>>(); // meh, but tableView.MakeView always returns null
		...
		public override nfloat GetRowHeight(NSTableView tableView, nint row)
		{
            ...
            if (cell == null)
            {
                _nativeCells.TryGetValue(templateId + row, out var weak);
                weak?.TryGetTarget(out cell);
            }
        ...
        public override NSView GetViewForItem(NSTableView tableView, NSTableColumn tableColumn, nint row)
		{
			...
			else if ((cachingStrategy & ListViewCachingStrategy.RecycleElement) != 0)
			{
                ...
                _nativeCells[templateId + row] = new WeakReference<Cell>(cell);
            }
            else
		...
		nfloat CalculateHeightForCell(NSTableView tableView, Cell cell)
		{
			...
			if (List.RowHeight == -1 && viewCell?.View != null)
			{
                ...
                */
                ...
                /*
				target.ClearValue(App.RenderProperty);
				foreach (var descendant in target.Descendants())
					descendant.ClearValue(App.RenderProperty);
                */
